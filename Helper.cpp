...
//++++++++++++++++++++++++++++++++++++++++EVENT++++++++++++++++++++++++++++++++++++++++++//
Event::Event(uint32_t len, uint32_t event_no, uint8_t event_type, const std::vector<char> &event_data)
{
    bytes.resize(13 + event_data.size());
    uint32_t l = htobe32(len);
    uint32_t e = htobe32(event_no);

    memcpy(&bytes[0], (char *)&l, 4);
    memcpy(&bytes[4], (char *)&e, 4);
    bytes[8] = event_type;
    memcpy(&bytes[9], &event_data[0], event_data.size());

    uint32_t c = htobe32(crc(bytes.size() - 4));
    memcpy(&bytes[9 + event_data.size()], (char *)&c, 4);
}

///////////////////////////////////////////CRC/////////////////////////////////////////////
uint32_t Event::crc(size_t size)
{
    const uint8_t *p = (uint8_t *)&bytes[0];
    uint32_t crc;
    crc = ~0U;
    while (size--)
...
////////////////////////////////GET BYTES///////////////////////////////////////////////
std::vector<char> Event::getBytes() const
{
    return bytes;
}
